...
pub fn from_yaml_path(file_path: &Path) -> Result<RawRcfile, RcfileError> {
  fs::read_to_string(file_path)
    .map_err(RcfileError::FileReadFailed)
    .and_then(|contents| serde_yaml::from_str::<RawRcfile>(&contents).map_err(RcfileError::YamlParseFailed))
}

pub fn try_from_yaml_candidates(cli: &Cli) -> Option<Result<RawRcfile, RcfileError>> {
